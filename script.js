...
const changeThemeBtns = Array.from(document.querySelectorAll('.change-theme'));
...
fetch(`https://wallpaper-api-zhirrr.vercel.app/api/technology`)
  ...
  .then(results => {
    showWallpaperResults(results, 'technology', 'Technology');
  });
  // .catch(error => {
  ...
  category.addEventListener('click', function(e) {
    ...
    if (e.target.dataset.category === 'technology' && e.target.classList.contains('current')) {
      fetch(`https://wallpaper-api-zhirrr.vercel.app/api/technology`)
        .then(response => response.json())
        .then(results => {
          showWallpaperResults(results, 'technology', e.target.textContent);
        });
        // .catch(error => {
        //   cardRow.innerHTML = showError();
        // });
      ...
      fetch(`https://wallpaper-api-zhirrr.vercel.app/api/${thisDataset}`)
        .then(response => response.json())
        .then(results => {
          showWallpaperResults(results, thisDataset, e.target.textContent);
        });
        // .catch(error => {
        //   cardRow.innerHTML = showError();
        // });
...
// FUNCTION
function showWallpaperResults(results, category, categoryTitle) {
//   if (results.status === 'ok') {
    // wallpaperDesc.textContent = 'Technology';
    // totalWall.textContent = result.length;
    const result = results.results;
    mainInfo.innerHTML = showMainInfo(categoryTitle, result.length);
    cardWall = '';

    result.forEach((wallpaper, index) => {
      // checkReadyImg(wallpaper, category, index);
      cardWall += showWallpaper(wallpaper, category, index);
    });
